...
def data_cleaning(data: pd.DataFrame):
    ...
    return (
        ...
        .assign(
            ...
            # Time based columns
            order_time = lambda x: pd.to_datetime(x['order_time'],errors='coerce',format='mixed'),
            ...
            multiple_deliveries=lambda x:x['multiple_deliveries'].replace('NaN ',np.nan).astype(float),
...
def time_of_day(ser):
    """
    Categorizes the hour part of a datetime series into time periods.
    """
    return pd.cut(
        ser,
...
def create_distance_type(data: pd.DataFrame):
  """
  Adds a 'distance_type' column to a DataFrame by binning the 'distance' column.
  """
  return (
      data
      .assign(
